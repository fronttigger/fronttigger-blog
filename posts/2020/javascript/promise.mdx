...
description: '약속의 중요성' # required
thumbnailImg: '/thumbnail/2020/11/javascript/promise.png' # required
---

## 프로미스(Promise)

**비동기 작업**을 순차적으로 진행할 수 있도록 도와줍니다. `ES6`부터 추가되었습니다.
...
비동기 코드를 처리하고 resolve();를 실행하면 **이행 상태**가 됩니다.
...
Promise 객체를 p에 담고, resolve 된 시점인 fulfilled 상태에서 `.then`으로 넘어가게 되고 .then 안에 있는 **콜백 함수**가 실행됩니다.  
즉 이 코드는 .then이 1초 후에 실행되게 될 것입니다.
